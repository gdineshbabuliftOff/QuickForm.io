...
export default function PublicFormPage() {
    ...
    const previewMode: PreviewMode = 'desktop'; // Fixed to 'desktop' for public view
                    ...
                    const normalizedPages: FormPage[] = (data.pages && data.pages.length > 0)
                        ? data.pages.map((page: FormPage) => ({
                            ...
                            fields: page.fields.map((field: FormField) => ({
                                ...
                                styles: {
                                    ...defaultFieldStyles, ...field.styles,
    ...
    const evaluateConditions = useCallback((conditions: FieldCondition[] = [], logic: ConditionLogic = 'and'): boolean => {
        ...
        const conditionChecks = conditions.map(cond => {
            ...
            const isTargetEmpty = valueToCompare === undefined || valueToCompare === null || valueToCompare === '' ||
                (Array.isArray(valueToCompare) && valueToCompare.length === 0) ||
                (valueToCompare instanceof FileList && valueToCompare.length === 0);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (currentPageIndex === form.pages.length - 1 || form.pages.length === 1) {
    ...
    // Define getCalculatedWidth here, making it accessible
    const getCalculatedWidth = useCallback((fieldWidth: FieldWidth | string | undefined): string => {
        // We need 'effectiveFormStyles.gap' which is dependent on 'form' state.
        // This function will be called within renderFieldElement, which has access to 'effectiveFormStyles'.
        // For calls outside renderFieldElement (like in the main map), we'll pass it the current form's effective gap.
        const currentGap = form?.styles[previewMode]?.gap ?? form?.styles.gap ?? defaultStyles.gap;

        if (!fieldWidth) return '100%';
        if (fieldWidth === '100%' || fieldWidth === '50%' || fieldWidth === '33.33%') {
            return `calc(${fieldWidth} - ${fieldWidth === '100%' ? 0 : currentGap}px)`;
        }
        return String(fieldWidth); // Assume custom width is already a valid CSS width
    }, [form, previewMode]); // Depend on form and previewMode to recalculate if styles change
    ...
    const renderFieldElement = (field: FormField): JSX.Element | null => {
        ...
        const combinedFieldStyle: React.CSSProperties = {
            color: baseFieldStyles.color || currentPageStyles.textColor || globalStyles.textColor,
            backgroundColor: baseFieldStyles.backgroundColor || currentPageStyles.fieldBackgroundColor || globalStyles.fieldBackgroundColor,
            borderColor: isErrored ? '#EF4444' : (baseFieldStyles.borderColor || currentPageStyles.fieldBorderColor || globalStyles.fieldBorderColor),
        ...
        // Get the responsive width for the current field
        const currentFieldWidth = responsiveFieldStyles.width || field.width;
        ...
        switch (field.type) {
            ...
            case 'ordered-list': return <div style={{ width: getCalculatedWidth(currentFieldWidth) }} ref={el => { if (field.name) fieldRefs.current[field.name] = el; }}>{fieldLabel}<ol className="list-inside pl-4" style={{ ...combinedFieldStyle, listStyleType: baseFieldStyles.listStyleType }}>{field.options?.map((opt, i) => <li key={i}>{opt.label}</li>)}</ol></div>;
            case 'unordered-list': return <div style={{ width: getCalculatedWidth(currentFieldWidth) }} ref={el => { if (field.name) fieldRefs.current[field.name] = el; }}>{fieldLabel}<ul className="list-inside pl-4" style={{ ...combinedFieldStyle, listStyleType: baseFieldStyles.listStyleType }}>{field.options?.map((opt, i) => <li key={i}>{opt.label}</li>)}</ul></div>;
            case 'text':
            case 'email':
            case 'date':
            case 'password':
            ...
            case 'color':
                return (
                    <div style={{ width: getCalculatedWidth(currentFieldWidth) }}>
                        {fieldLabel}
            ...
            case 'textarea':
                return (
                    <div style={{ width: getCalculatedWidth(currentFieldWidth) }}>
                        {fieldLabel}
                        <textarea name={field.name} id={field.id} value={formData[field.name || ''] || ''} placeholder={field.placeholder} required={isRequired} onChange={handleInputChange} style={combinedFieldStyle} className={`${baseInputClasses} ${errorInputClasses}`} ref={el => { if (field.name) fieldRefs.current[field.name] = el; }} />
                        {errorDisplay}
                    </div>
                );
            case 'select':
                return (
                    <div style={{ width: getCalculatedWidth(currentFieldWidth) }}>
                        {fieldLabel}
                        <select name={field.name} id={field.id} value={formData[field.name || ''] || ''} required={isRequired} onChange={handleInputChange} style={combinedFieldStyle} className={`${baseInputClasses} ${errorInputClasses}`} ref={el => { if (field.name) fieldRefs.current[field.name] = el; }}>
            ...
            case 'checkbox':
                return (
                    <div style={{ width: getCalculatedWidth(currentFieldWidth) }}>
                        {fieldLabel}
                        <div className={`flex gap-4 ${field.layout === 'horizontal' ? 'flex-row flex-wrap' : 'flex-col'}`} ref={el => { if (field.name) fieldRefs.current[field.name] = el; }}>
                                ...
                                <div key={i} className="flex items-center gap-2">
                                    {controlStyle === 'switch' ? (
                                        ...
                                        <label htmlFor={`${field.id}_${i}`} className="flex items-center cursor-pointer">
                                            <div className={`w-5 h-5 border-2 rounded flex items-center justify-center ${Array.isArray(formData[field.name || '']) && formData[field.name || ''].includes(opt.value) ? 'bg-indigo-600 border-indigo-600' : (isErrored ? 'bg-red-500 border-red-500' : 'bg-gray-700 border-gray-500')}`}>
                                                <CheckIcon size={14} className={`${Array.isArray(formData[field.name || '']) && formData[field.name || ''].includes(opt.value) ? 'text-white' : 'text-transparent'}`} />
                                            </div>
                                    ...
                                    <label htmlFor={`${field.id}_${i}`} style={{ color: combinedFieldStyle.color }} className={`${controlStyle === 'line-through' && Array.isArray(formData[field.name || '']) && formData[field.name || ''].includes(opt.value) ? 'line-through' : ''}`}>
                                        {opt.label}
                                    </label>
                                </div>
                            ))}
                        ...
                        <div className={`flex gap-4 ${field.layout === 'horizontal' ? 'flex-row flex-wrap' : 'flex-col'}`} ref={el => { if (field.name) fieldRefs.current[field.name] = el; }}>
                            {field.options?.map((opt, i) => (
                                <div key={i} className="flex items-center gap-2">
                                    ...
                                    <label htmlFor={`${field.id}_${i}`} style={{ color: combinedFieldStyle.color }}>
                                        {opt.label}
                                    </label>
                                </div>
                            ))}
            ...
            case 'signature':
                return (
                    <div style={{ width: getCalculatedWidth(currentFieldWidth) }}>
                        {fieldLabel}
                        <div className="w-full h-32 bg-gray-100 border-2 border-dashed rounded-lg flex items-center justify-center" style={{ borderColor: isErrored ? '#EF4444' : (baseFieldStyles.borderColor || currentPageStyles.fieldBorderColor || globalStyles.fieldBorderColor) }} ref={el => { if (field.name) fieldRefs.current[field.name] = el as HTMLDivElement; }}>
                            <SignatureIcon className="text-gray-500" size={40} />
                        </div>
                        <p className="text-xs text-gray-500 mt-1">Signature pad is for demonstration only.</p>
                        {errorDisplay}
                    </div>
                );
            case 'rating':
                return (
                    <div style={{ width: getCalculatedWidth(currentFieldWidth) }}>
                        {fieldLabel}
                        <div className="flex items-center" ref={el => { if (field.name) fieldRefs.current[field.name] = el; }}>
                            {[...Array(field.maxRating || 5)].map((_, i) => {
                                ...
                                return (
                                    <StarIcon
                                        ...
                                        fill={ratingValue <= (formData[field.name || ''] || 0) ? 'currentColor' : 'none'}
                                        color={combinedFieldStyle.color}
                                        onClick={() => handleInputChange({ target: { name: field.name || '', value: String(ratingValue), type: 'radio' } } as React.ChangeEvent<HTMLInputElement>)}
                                    />
                                );
                            })}
                        </div>
            ...
            case 'image':
                return <img src={field.src || 'https://via.placeholder.com/400x200'} alt={field.label} style={{ ...combinedFieldStyle, width: getCalculatedWidth(currentFieldWidth), height: 'auto', objectFit: 'cover' }} ref={el => { if (field.name) fieldRefs.current[field.name] = el as HTMLImageElement; }} />;
            case 'name':
            case 'address':
            case 'section':
                return (
                    <div className="w-full rounded-lg border-2" style={{ ...combinedFieldStyle, padding: baseFieldStyles.padding, borderColor: isErrored ? '#EF4444' : (baseFieldStyles.borderColor || currentPageStyles.fieldBorderColor || globalStyles.fieldBorderColor) }} ref={el => { if (field.name) fieldRefs.current[field.name] = el as HTMLDivElement; }}>
                        <h3 className="text-xl font-semibold mb-4" style={{ color: combinedFieldStyle.color }}>{field.label}</h3>
                        <div className="flex flex-wrap" style={{ rowGap: `${effectiveFormStyles.gap}px`, columnGap: `${effectiveFormStyles.gap}px` }}>
                            {field.fields?.map(subField => {
                                const subFieldResponsiveWidth = subField.styles?.[previewMode]?.width || subField.width;
                                const renderedSubField = renderFieldElement(subField);
                                if (!renderedSubField) return null;
                                return <div key={subField.id} style={{ width: getCalculatedWidth(subFieldResponsiveWidth) }}>{renderedSubField}</div>;
                            })}
                        </div>
                        {errorDisplay}
                    </div>
            ...
            <div className="w-full max-w-4xl">
                <div className="bg-white/20 p-6 sm:p-8 rounded-xl shadow-2xl backdrop-blur-lg border border-white/30">
                    ...
                    {submissionStatus === 'error' && !generalError && (
                        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">
                            <strong className="font-bold">Error!</strong>
                            <span className="block sm:inline"> {form.settings.submitErrorMessage || "Submission failed. Please try again."}</span>
                        </div>
                    )}
                    ...
                    {showThankYouPage ? (
                        <div className="text-center">
                            ...
                            <h2 className="text-3xl font-bold mt-4" style={{ color: effectiveFormStyles.textColor }}>{form.settings.submitSuccessMessage || "Thank You!"}</h2>
                            {form.settings.thankYouPageContent.length > 0 ? (
                                <div className="mt-2 text-lg">
                                    {form.settings.thankYouPageContent.map(field => renderFieldElement(field))}
                                </div>
                            ) : (
                                <p className="mt-2 text-lg" style={{ color: effectiveFormStyles.textColor }}>Your response has been successfully submitted.</p>
                            )}
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-wrap" style={{ rowGap: `${effectiveFormStyles.gap}px`, columnGap: `${effectiveFormStyles.gap}px` }}>
                            {currentPageData.fields.map(field => {
                                ...
                                return (
                                    <div key={field.id} style={{ width: getCalculatedWidth(responsiveWidth) }}>
                                        {renderedField}
                                    </div>
                                );
                            })}
                            ...
                            <div className="w-full mt-8 flex"
                                ...
                                {isMultiPageForm && !isFirstPage && (
                                    <button type="button" onClick={handlePreviousPage} style={prevButtonStyle} className="font-bold py-2 px-6 rounded-lg cursor-pointer shadow-lg hover:opacity-90 transition-opacity">
                                ...
                                {isMultiPageForm && (
                                    <>
                                        ...
                                        {isLastPage && (
                                            <button type="submit" disabled={isSubmitting} style={submitButtonStyle} className={`font-bold py-2 px-6 rounded-lg cursor-pointer shadow-lg hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-wait ${isFirstPage ? '' : 'ml-auto'}`}>
                                                {isSubmitting ? 'Submitting...' : effectiveFormStyles.buttonText}
                                            </button>
                                        )}
                                    </>
                                )}
                                ...
                                {!isMultiPageForm && (
                                    <button type="submit" disabled={isSubmitting} style={submitButtonStyle} className="font-bold py-2 px-6 rounded-lg cursor-pointer shadow-lg hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-wait">
                                        {isSubmitting ? 'Submitting...' : effectiveFormStyles.buttonText}
                                    </button>
                                )}
                            </div>
