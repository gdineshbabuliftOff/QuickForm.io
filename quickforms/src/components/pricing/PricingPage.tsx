...
import { useAuth } from '@/hooks/useAuth'; // Assuming you have this hook for user status
...
const PricingTier: React.FC<PricingTierProps> = ({
  ...
  const highlightClasses = isHighlighted
    ? 'bg-indigo-700 border-indigo-500 shadow-xl scale-105'
  ...
  return (
    <div className={`flex flex-col p-8 rounded-2xl border transition-all duration-300 ${highlightClasses}`}>
      ...
      <Link href={buttonLink} className={`block w-full text-center py-3 rounded-lg font-semibold transition-colors duration-200 ${buttonClasses}`}>
        {buttonText}
      </Link>
    </div>
  );
...
const PricingPage: React.FC = () => {
    const { user, loading } = useAuth(); // Use your auth hook to check if user is logged in
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-lg shadow-lg">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            ...
            <Link href="/pricing" className="text-indigo-400 transition-colors">Pricing</Link> {/* Highlight current page */}
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          ...
          <PricingTier
            ...
            features={[
              ...
              'Premium Templates',
              'Advanced Fields (Signature, File Upload)',
              'Conditional Logic',
              'Custom CSS',
              'API Integrations',
              'Priority Support',
            ...
            buttonLink="/signup?plan=pro" // Example: pass plan as query param
          />
          <PricingTier
            name="Business"
            price="$49"
